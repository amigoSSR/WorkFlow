...
<head>
    <meta charset="utf-8">
    <title>Master Weekly Checkup Report</title>
    <style>
        body {
...
<body>
    ...
    @foreach($projectsData as $index => $data)
        ...
        <table class="info-table">
            ...
            <tr>
                ...
                <td colspan="3">{{ $data['project']->users->pluck('name')->join(', ') }}</td>
            </tr>
        </table>
        ...
        <table class="stats-container">
            <tr>
                <td>
                    <div class="stat-value">{{ $data['stats']['total_roadmaps'] }}</div>
                    <div class="stat-label">Total Roadmap</div>
                </td>
                <td>
                    <div class="stat-value">{{ $data['stats']['completed_milestones'] }} / {{ $data['stats']['total_milestones'] }}</div>
        ...
        <div class="section-title">Aktivitas Mingguan (Diary) Semua User</div>
        @if($data['activities']->count() > 0)
            @foreach($data['activities'] as $activity)
                <div class="activity-item">
                    <div>
                        <strong>{{ $activity->title }}</strong> 
                        <span style="font-size: 10px; color: #6e7979;">- Kategori: {{ $activity->category }} | Oleh: {{ optional($activity->user)->name }}</span>
                    </div>
    ...
    <div style="margin-top: 40px; text-align: center; color: #6e7979; font-size: 10px; border-top: 1px solid #bdc9c8; padding-top: 10px;">
        Dokumen Master Weekly Report ini dibuat secara otomatis oleh sistem CollabOps pada {{ $report_date->format('d M Y H:i') }}.
    </div>
